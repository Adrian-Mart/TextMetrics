...
def download(skip, output, name, yes, base_dir, set_dir=None):
    ...
    if name:
        ...
        cache_file_path = os.path.join(base_dir, output, 'raw.cache')
        with open(os.path.join(base_dir, set_dir), 'r', encoding='UTF-8') as file:
            titles = file.readlines()
    else:
        with open(os.path.join(base_dir, 'data', 'titles.txt'), 'r', encoding='utf-8') as file:
            titles = file.readlines()
...
if __name__ == "__main__":
    ...
    download(args.not_skip is not None, args.output, args.name, args.yes, args.base_dir)